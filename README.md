...
Version tags look like `X.Y.Z` where `X.Y` indicate the supported major and minor version of Netbox
and `Z` is the version of `netbox-dhcp` within that scope.
...
```
pip install git+https://github.com/ChrisPortman/netbox-dhcp.git@<tag>
```
...
1. If the MAC address is associated with an interface, that interface has an IP which is in the same
   subnet on which the DHCP request originated. The interface IP address is used.
